...
/* --- tester --- */
	...
	public static void main (String argv[]) throws Exception
	{
		...
		for (int i=0; i < api.analysisResults.size(); i+=2) {
			System.out.println("Doc {" + api.analysisResults.get(i) + "} = {" + api.analysisResults.get(i+1) + "}");
		}
...
/* --- Public parts --- */
	...
	public void addDocumentsFromCSV(String path, String canonicizers[]) throws JgaapAPIException
	{
		...
		if (rows == null) {
			thrower(exception_types.BAD_FILE, "readCSV() choked on that path");
		}
		...
		for (int i=0; i<rows.size(); i++) {
			...
			if (doc.size() < 2) {
				thrower(exception_types.BAD_FILE, "Row (index "+i+") has too few columns");
			}
	...
	public void addDocument(String path, String author, String canonicizers[]) throws JgaapAPIException
	{
		...
		if (path == null) {
			thrower(exception_types.BAD_ARGUMENT, "path is null");
		}
		...
		for (int i=0; i<canonicizers.length; i++) {
			...
			if (c==null) {
				thrower(exception_types.BAD_CANONICIZER, "Canonicizer \"" + canonicizers[i] + "\" probably doesn't exist");
			}
		...
		try {
			...
			rethrow(exception_types.UNKNOWN, e, "new Document("+path+", "+author+") threw an exception.");
		}
		...
		try {
			...
			rethrow(exception_types.UNKNOWN, e, "Jgaap encountered an interal error related to Language");
		}
	...
	public void selectEventSet(String eventSet, boolean mostCommon) throws JgaapAPIException
	{
		...
		Set<String> possibleESets = driver.getAllEventSets();

		if (!possibleESets.contains(eventSet.toLowerCase())) {
			thrower(exception_types.BAD_EVENT_SET, eventSet+" is an invalid event set.");
		}
	...
	public void selectAnalysisMethod(String analysisMethod) throws JgaapAPIException
	{
		...
		if (!(anaMethods.containsKey(analysisMethod.toLowerCase())
			...
			thrower(exception_types.BAD_ANALYSIS_METHOD, analysisMethod+" is an invalid analysis method.");
		}
...
//		}
		...
		// Apply the event set
		try {
			driver.addEventDriver(chosenEventSet);
			//driver.createEventSet();
		} catch (Exception e) {
			rethrow(exception_types.UNKNOWN, e, "An exception was thrown within JGAAP while in createEventSet("+chosenEventSet+", "+chosenEventSet_mostCommon+")");
		}
		...
		{
			thrower(exception_types.UNKNOWN, "An internal error occured within JGAAP while in createEventSet("+chosenEventSet+", "+chosenEventSet_mostCommon+")");
		}
		...
		try {
			...
			rethrow(exception_types.UNKNOWN, e, "Weird internal error (setParameter(divergenceOption) failed (???))");
		}
		...
		try {
			driver.addAnalysis(chosenAnalysisMethod);
			analysisResults = driver.executeRaw();
		} catch (Exception e) {
			rethrow(exception_types.UNKNOWN, e, "An internal error ccured within JGAAP while performing statistical analysis ("+chosenAnalysisMethod+")");
		}
	...
	public JgaapAPI()
	{
		...
		anaMethods = new HashMap<String, AnalysisDriver>();
		for(AnalysisDriver analysisDriver: AutoPopulate.getAnalysisDrivers()){
			anaMethods.put(analysisDriver.displayName().toLowerCase(), analysisDriver);
		}
		distMethods  = new HashMap<String, DistanceFunction>();
		for(DistanceFunction distanceFunction: AutoPopulate.getDistanceFunctions()){
			distMethods.put(distanceFunction.displayName().toLowerCase(), distanceFunction);
		}
	}
	...
	//public exception_types exception_types;
...
/* --- Internal parts --- */
	...
	{
		if (isInvalid) {
			thrower(exception_types.OBJECT_INVALID, "This instance is invalid (because it previously threw an exception).");
		}
	}
	...
	{
		if (didExecute) {
			thrower(exception_types.ALREADY_EXECUTED, "This instance already had execute() called. Only its results may be accessed.");
		}
	}
