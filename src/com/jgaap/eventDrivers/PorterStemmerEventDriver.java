...
/**
 * Applies Porter's stemming algorithm (the Porter Stemmer) to produce
 * just the stems of the underlying words or word sets.
 * 
 * Stems, e.g. : farms, farmed, farming should all become just "farm"
 * 
 * Porter's algorithm is freely licenced for all uses; I got the code
 * from Martin Porter himself at
 *    http://tartarus.org/~martin/PorterStemmer/java.txt
 * 
 * A few comments:
 *      1) This algorithm is (obviously) English-only
 *      2) There are better stemming algorithms out there (TODO) such as
 *      the Snowball algorithm.
 *      3) The code only works for lower case characters.  Should not be
 *      a problem most of the time.  I have put in an ugly hack to address.
 *      4) Ditto punctuation.  I have NOT put in a hack to address
 */
public class PorterStemmerEventDriver extends EventDriver {
	...
	for (int i = 0; i < raw.size(); i++) {
	    ...
	    cooked.events.add( new Event( new String(b,0,
			theStemmer.getResultLength()) ) );
	}
	return cooked;
    }
