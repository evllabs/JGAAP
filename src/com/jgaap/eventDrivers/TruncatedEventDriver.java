...
/**
 * Truncates Events to shorter strings -- i.e. "hello" becomes "he"
 * Useful for binning NumericEventSets among other things
 */
public class TruncatedEventDriver extends EventDriver {
	...
	if (es instanceof NumericEventSet)
	    ...
	    if (s.length() > length) {
		try {
		    newEs.events.add( new Event(s.substring(0,length)));
		} catch (Exception e) {
		    System.out.println("Error in truncating "+s);
		    newEs.events.add( new Event(s) );
		}
	    } else 
		newEs.events.add( new Event(s) );
