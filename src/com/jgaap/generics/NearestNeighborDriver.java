	...
	public String analyze(EventSet unknown, List<EventSet> known) {
		...
		for (int i = 0; i < known.size(); i++) {
			...
			if (!meanAuthorDistance) {
				...
				Vector<Double> tmp = authorDistances.get(known.get(i)
						.getAuthor());
				if (tmp==null)
					tmp = new Vector<Double>();
				tmp.add(current);
				authorDistances.put(known.get(i).getAuthor(), tmp);
			}
		}
		if (meanAuthorDistance) {
			authorArray = new String[authorDistances.size()];
			distArray = new double[authorDistances.size()];
