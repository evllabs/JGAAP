	...
	public List<EventSet> cull(List<EventSet> eventSets) {
	     ...
	     String informative;
	       ...
	       for(EventSet eventSet : eventSets){
	       	...
	       	for(Pair<Event,Integer>event : eventFrequencies){
	       		Event e = event.getFirst();
	       		percentage = new BigDecimal(hist.getRelativeFrequency(e));
	    	   for(EventHistogram eventHistogram : eventHistograms){	
	    		   int mi = eventHistogram.getAbsoluteFrequency(e);
	    		   /* 
	    	   ...
	    	   infoGain.add(new Pair<Event,Double>(event.getFirst(), res));	 
	       ...
	       /*
	        * IGSet holds the k first events in infoGain.
	        * infoGain is already sorted by most or least informative
	        */
	       List <Event> IGSet = new ArrayList<Event>();
	       for(int i = minPos; i<minPos+numEvents; i++){
	    	   IGSet.add(infoGain.get(i).getFirst());
	       }
	    //   System.out.println(IGSet);
	       for(EventSet oneSet : eventSets) {
